...
const Events = () => {
  return (
    <div className="md:px-10 md:py-10 px-4 py-10 bg-[#E7F0F9] flex flex-col">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 xl:grid-cols-4 gap-4 w-full">
        {eventsData.map((event, index) => (
          <div
            key={index}
            className="flex flex-col sm:flex-row bg-white rounded-lg overflow-hidden group min-h-[160px] w-full"
          >
            {/* Image Column */}
            <div className="relative w-full sm:w-[150px] h-[200px] sm:h-auto flex-shrink-0">
              <Image
                src={event.imgUrl}
                alt={event.title}
                fill
